...
type StorageClassClient interface {
	...
	Create(context.Context, *storagev1.StorageClass, metav1.CreateOptions) (*storagev1.StorageClass, error)
...
func (h *Helper) SetAuth() error {
	...
	h.dynamicClient, err = dynamic.NewForConfig(h.Auth.RestConf)
	if err != nil {
		return err
	}

	discoveryClient := memory.NewMemCacheClient(h.clientset.Discovery())
	h.discoveryMapper = restmapper.NewDeferredDiscoveryRESTMapper(discoveryClient)
	h.apiExtensionClientset, err = apiextensionsclient.NewForConfig(h.Auth.RestConf)
	if err != nil {
		return err
	}
...
func (h *Helper) SetCustomResourceDefinitionClient() {
	h.CustomResourceDefinitionClient = h.apiExtensionClientset.ApiextensionsV1().CustomResourceDefinitions()
}
...
func (h *Helper) CreateStorageClass(storageClass *storagev1.StorageClass) (*storagev1.StorageClass, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	return h.StorageClassClient.Create(ctx, storageClass, metav1.CreateOptions{})
}
...
func (h *Helper) CreateSecret(secret *corev1.Secret) (*corev1.Secret, error) {
	ctx, cancel := context.WithTimeout(wait.CtxSeconds(5))
	defer cancel()
	return h.SecretClient.Create(ctx, secret, metav1.CreateOptions{})
}
...
func (h *Helper) ApplyDynamicResource(reader io.Reader) error {
	...
	for {
		...
		restMapping, err := h.discoveryMapper.RESTMapping(gvk.GroupKind(), gvk.Version)
		if err != nil {
			return err
		}
		...
		_, err = h.dynamicClient.Resource(gvr).Namespace(namespace).Apply(ctx, obj.GetName(), obj, applyOpts)
		if err != nil {
			return fmt.Errorf("failed to apply dynamic resource(%v)", err)
		}
	}
...
func (h *Helper) ListCustomResourceDefinition() (*apiextensionsv1.CustomResourceDefinitionList, error) {
	ctx, cancel := context.WithTimeout(wait.CtxSeconds(5))
	defer cancel()
	return h.CustomResourceDefinitionClient.List(ctx, metav1.ListOptions{})
}
...
func (h *Helper) GetConfigMap(name string) (*corev1.ConfigMap, error) {
	ctx, cancel := context.WithTimeout(wait.CtxSeconds(5))
	defer cancel()
